...
class PathSimRec:
    ...
    def predict_next_by_sessionKNN(self, session, item_list, kNN):
        ...
        if len(session_relevance_vector) > 0:
            ...
            for s, score in sessions:
                articles = [a for a in self.G[s] if self.G[s][a]['edge_type'] == 'SA' and a not in item_list]
                if len(articles) > 0:
    ...
    def predict_next_by_SB(self, session, item_list, topN=True):

        similar_sessions = self.get_similar_sessions(session)

        articles = []
        for s in similar_sessions:
            articles.extend([a for a in self.G[s] if self.G[s][a]['edge_type'] == 'SA' and a not in item_list])
    ...
    def predict_next_by_UB(self, similar_users, item_list, g, topN=True):
