...
class DataImport:
    ...
    def import_user_click_data(self, data_path, adjust_pk_names=False, order_by_time=True, pk_item = 'pk_article'):
        '''
        ...
        user_ses_df = pd.read_csv(data_path, sep='\t', header=None)
        ...
        if adjust_pk_names == True:
            user_ses_df['pk_user'] = 'U' + user_ses_df['pk_user'].astype('str')
            user_ses_df['pk_session'] = 'S' + user_ses_df['pk_session'].astype('str')
            user_ses_df[pk_item] = 'A' + user_ses_df[pk_item].astype('str')
