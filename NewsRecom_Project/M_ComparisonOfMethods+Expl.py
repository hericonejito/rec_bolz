...
# pickle_name = 'Articles_LongSessions(4n)'
...
# results_dir = './Data/Results/Newest/Comparison/Expl/'
...
if file_or_pickle == 'F':
    ...
    nx.write_gpickle(gm.G, f'./Data/Pickles/'+pickle_name+'.gpickle')
...
pop = PopularityBasedRec(G, N) #baseline recommendation method = Most Populars
...
# ----- Meta paths for explained recommendations
PathCount_ASA = PathSimRec(N)
PathCount_ACA = PathSimRec(N)
PathCount_ALA = PathSimRec(N)
PathCount_AUA = PathSimRec(N)
...
PathCount_SCS = PathSimRec(N)
PathCount_SLS = PathSimRec(N)

PathCount_UAU = PathSimRec(N)
PathCount_UCU = PathSimRec(N)
PathCount_ULU = PathSimRec(N)
...
for tw_i, tw_iter in enumerate(tas.time_window_graph_list):
    ...
    for (s, s_datetime) in test_sessions:
        ...
        for i in range(MIN_ITEMS_N, len(articles)):
            ...
            methods = [pop_rec,
                       rwr_sal_s_rec, rwr_sa_s_rec, rwr_usacl_s_rec,
                       sknn_rwr_usacl_rec,
